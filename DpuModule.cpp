...
static int dpu_device_update_display_size(dpu_device_t *dev,
					uint32_t w, uint32_t h) {
  GET_DPU_RETURN_ERROR_IF_NULL(dev);

  if (w == 0 || h == 0) {
    ALOGE("update display size params error");
    return -1;
  }

  return dpu->updateDisplaySize(w, h);
}
...
static int dpu_device_open(const struct hw_module_t *module, const char *name,
  ...
  dpu->dpu_device_t::updateDisplaySize = dpu_device_update_display_size;
