...
typedef struct dpu_device {
  ...
  int (*updateDisplaySize)(struct dpu_device *device, uint32_t w, uint32_t h);
} dpu_device_t;
...
class DpuDevice : public dpu_device_t {
 public:
  ...
  int64_t GetProperty(uint32_t obj_id, uint32_t obj_type, const char *prop_name,
                      int fd);
  int updateDisplaySize(uint32_t w, uint32_t h);
